...
class BaseRegistry<Registrable> {
	...
	public function register(id:String, object:Registrable, ?force:Bool = false):Registrable {
		if (this.has(id) && !force) {
			throw '[$name] Tried registering "$id" but it already exists, use a different ID.';
			return null;
		}
		if (locked) {
			trace('[$name] You cannot register anything new if the registry is locked (for "$name")');
			return null;
		}
	...
	public function get(id:String):Registrable {
		if (!this.has(id)) {
			trace('[$name] Registry does not have "$id" (does it need to be registered?)');
			return null;
		}
		return entries.get(id);
	}
	...
	public function getFromIndex(idx:Int):Registrable {
		...
		trace('[$name] Entry at index $idx not found.');
		return null;
	}
	...
	public function unregister(id:String):Bool {
		...
		if (locked) {
			trace('[$name] You cannot unregister anything new if the registry is locked (for "$name")');
			return result;
		}
		if (this.has(id)) {
			result = true;
